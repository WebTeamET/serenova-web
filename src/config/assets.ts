const BASE = '/assets/images'

export const ASSETS = {
  // Banner / Hero
  bannerTree: '/banner-tree.png',
  bannerDetailLeaf: '/banner-detail-leaf.png',
  bannerDownArrow: '/banner-down-arrow.svg',

  // About / Best
  aboutLeaf: '/about-leaf.png',
  bestBird: '/best-bird.png',
  bestTree: '/best-tree.png',
  bestPause: '/best-pause.png',
  videoPlaceholder: '/video-placeholder.webp',

  // Services
  serviceArrowNext: '/service-arrow-next.svg',

  // Tours
  tourLeaf: '/tour-leaf.png',
  tourTree: '/tour-tree.png',

  // Offers
  offerLeaf: `${BASE}/offer-leaf.png`,
  offerBench: `${BASE}/offer-bench.png`,

  // Explore
  exploreLeaf: `${BASE}/explore-leaf.png`,

  // Booking Banner
  subtitleTreeWhite: `${BASE}/subtitle-tree-white.svg`,

  // Testimonials
  testBigLeaf: `${BASE}/test-big-leaf.png`,
  testQuote: `${BASE}/test-quote.png`,
  testLeaf: `${BASE}/test-leaf.png`,
  swiperPrev: `${BASE}/swiper-prev.svg`,
  swiperNext: `${BASE}/swiper-next.svg`,

  // Footer
  footerLeafLeft: `${BASE}/footer-leaf-left.png`,
  footerLeafRight: `${BASE}/footer-leaf-right.png`,
  footerPlaneIcon: `${BASE}/footer-plane-icon.svg`,
  footerSubscribeIcon: `${BASE}/footer-subscribe-icon.svg`,

  // Social icons
  social: {
    facebook: `${BASE}/facebook.svg`,
    twitter: `${BASE}/twitter.svg`,
    instagram: `${BASE}/instagram.svg`,
    dribble: `${BASE}/dribble.svg`,
  },
} as const
